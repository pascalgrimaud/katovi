...
import io.github.pascalgrimaud.katovi.config.KafkaProperties;
...
class KatoviKafkaResourceIT {
    ...
    void setup() {
        ...
        KatoviKafkaResource kafkaResource = new KatoviKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/katovi-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/katovi-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
